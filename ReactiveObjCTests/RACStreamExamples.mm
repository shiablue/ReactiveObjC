	...
	sharedExamples(RACStreamExamples, ^(QCKDSLSharedExampleContext exampleContext) {
		...
		describe(@"-bind:", ^{
			it(@"should return the result of binding a single value", ^{
				RACStream *stream = [[streamClass return:@0] bind:(RACStreamBindHandler)^{
			...
			it(@"should concatenate the result of binding multiple values", ^{
				...
				RACStream *stream = [baseStream bind:(RACStreamBindHandler)^{
					return ^(NSNumber *value, BOOL *stop) {
						NSNumber *newValue = @(value.integerValue + 1);
						return [streamClass return:newValue];
					};
			...
			it(@"should terminate immediately when returning nil", ^{
				RACStream *stream = [infiniteStream bind:(RACStreamBindHandler)^{
			...
			it(@"should be restartable even with block state", ^{
				...
				RACStream *countingStream = [baseStream bind:(RACStreamBindHandler)^{
			...
			it(@"should be interleavable even with block state", ^{
				...
				RACStream *zipped = [streamClass zip:@[ countingStream, anotherStream ] reduce:(RACGenericReduceBlock)^(NSNumber *v1, NSNumber *v2) {
		...
		describe(@"zip stream creation methods", ^{
			...
			describe(@"+zip:reduce:", ^{
				it(@"should reduce values", ^{
					RACStream *stream = [streamClass zip:threeStreams reduce:(RACStreamBindHandler)^ NSString * (id x, id y, id z) {
				...
				it(@"should truncate streams", ^{
					...
					RACStream *stream = [streamClass zip:streams reduce:(RACStreamBindHandler)^ NSString * (id w, id x, id y, id z) {
				...
				it(@"should handle multiples of the same stream", ^{
					...
					RACStream *stream = [streamClass zip:streams reduce:(RACStreamBindHandler)^ NSString * (id x1, id x2, id y1, id z1, id y2, id z2) {
		...
		it(@"should reduce tuples", ^{
			...
			RACStream *reduced = [stream reduceEach:(RACReduceBlock)^(NSString *a, NSString *b) {
