...
describe(@"-rac_valuesForKeyPath:observer:", ^{
	...
	itBehavesLike(RACPropertySubscribingExamples, ^{
...
describe(@"+rac_signalWithChangesFor:keyPath:options:observer:", ^{
	describe(@"KVO options argument", ^{
		...
		beforeEach(^{
			...
			objectValueSignal = ^RACSignal * (NSKeyValueObservingOptions options) {
				return [[object rac_valuesAndChangesForKeyPath:@keypath(object, objectValue) options:options observer:self] reduceEach:(RACReduceBlock)^(id value, NSDictionary *change) {
					return change;
				}];
			};
		});
