...
describe(@"RACStream", ^{
	...
	itBehavesLike(RACStreamExamples, ^{
		return @{
...
describe(@"-bind:", ^{
	...
	beforeEach(^{
		...
		RACSignal *bind = [source bind:(RACSignalBindHandler)^{
	...
	it(@"should properly stop subscribing to new signals after error", ^{
		...
		RACSignal *bind = [signal bind:(RACSignalBindHandler)^{
	...
	it(@"should not retain signals that are subscribed", ^{
...
describe(@"+combineLatest:reduce:", ^{
	...
	it(@"should send nils for nil values", ^{
		...
		RACSignal *combined = [RACSignal combineLatest:@[ subject1, subject2, subject3 ] reduce:(RACGenericReduceBlock)^ id (id val1, id val2, id val3) {
			receivedVal1 = val1;
			receivedVal2 = val2;
			receivedVal3 = val3;
			return nil;
	...
	it(@"should send the return result of the reduce block", ^{
		RACSignal *combined = [RACSignal combineLatest:@[ subject1, subject2, subject3 ] reduce:(RACGenericReduceBlock)^(NSString *string1, NSString *string2, NSString *string3) {
	...
	it(@"should handle multiples of the same signals", ^{
		RACSignal *combined = [RACSignal combineLatest:@[ subject1, subject2, subject1, subject3 ] reduce:(RACGenericReduceBlock)^(NSString *string1, NSString *string2, NSString *string3, NSString *string4) {
	...
	it(@"should handle multiples of the same side-effecting signal", ^{
		...
		RACSignal *combined = [RACSignal combineLatest:@[ sideEffectingSignal, sideEffectingSignal, sideEffectingSignal ] reduce:(RACGenericReduceBlock)^(id x, id y, id z) {
...
describe(@"-setKeyPath:onObject:", ^{
	...
	itBehavesLike(RACPropertySignalExamples, ^{
...
describe(@"-flatten:", ^{
	...
	describe(@"when its max is 0", ^{
		...
		itBehavesLike(RACSignalMergeConcurrentCompletionExampleGroup, ^{
...
describe(@"+try:", ^{
	...
	it(@"should pass the value if it is non-nil", ^{
	...
	it(@"should ignore the error if the value is non-nil", ^{
		RACSignal *signal = [RACSignal try:^(NSError **error) {
	...
	it(@"should send the error if the return value is nil", ^{
...
it(@"should complete take: even if the signal is recursive", ^{
	RACSubject *subject = [RACSubject subject];
...
describe(@"+zip:", ^{
	...
	it(@"should handle signals sending values unevenly", ^{
		...
		[[RACSignal zip:@[ a, b, c ] reduce:(RACGenericReduceBlock)^(NSNumber *a, NSNumber *b, NSNumber *c) {
	...
	it(@"should handle multiples of the same side-effecting signal", ^{
		...
		RACSignal *combined = [RACSignal zip:@[ sideEffectingSignal, sideEffectingSignal ] reduce:(RACGenericReduceBlock)^ NSString * (id x, id y) {
...
describe(@"-groupBy:", ^{
	...
	it(@"should send completed in the order grouped signals were created.", ^{
		RACSubject *subject = [RACReplaySubject subject];
