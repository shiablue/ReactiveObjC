...
+ (void)configure:(Configuration *)configuration {
	...
	sharedExamples(RACKVOWrapperCollectionExamples, ^(QCKDSLSharedExampleContext exampleContext) {
		__block NSObject *target = nil;
		__block NSString *keyPath = nil;
...
describe(@"-rac_observeKeyPath:options:observer:block:", ^{
	describe(@"on simple keys", ^{
		...
		itBehavesLike(RACKVOWrapperExamples, ^{
			return @{
				RACKVOWrapperExamplesTargetBlock: targetBlock,
				RACKVOWrapperExamplesKeyPath: @keypath([[RACTestObject alloc] init], weakTestObjectValue),
		...
		itBehavesLike(RACKVOWrapperCollectionExamples, ^{
			return @{
				RACKVOWrapperCollectionExamplesTargetBlock: targetBlock,
				RACKVOWrapperCollectionExamplesKeyPath: @keypath([[RACTestObject alloc] init], orderedSetValue)
			};
		});
	});
	...
	describe(@"on composite key paths'", ^{
		describe(@"last key path components", ^{
			...
			itBehavesLike(RACKVOWrapperExamples, ^{
				return @{
					RACKVOWrapperExamplesTargetBlock: targetBlock,
					RACKVOWrapperExamplesKeyPath: @keypath([[RACTestObject alloc] init], strongTestObjectValue.weakTestObjectValue),
					RACKVOWrapperExamplesChangeBlock: changeBlock,
					RACKVOWrapperExamplesValueBlock: valueBlock,
					RACKVOWrapperExamplesChangesValueDirectly: @YES
				};
			...
			itBehavesLike(RACKVOWrapperCollectionExamples, ^{
				return @{
					RACKVOWrapperCollectionExamplesTargetBlock: targetBlock,
					RACKVOWrapperCollectionExamplesKeyPath: @keypath([[RACTestObject alloc] init], strongTestObjectValue.orderedSetValue)
				};
			});
		});
...
describe(@"rac_addObserver:forKeyPath:options:block:", ^{
	...
	context(@"automatically stops KVO on subclasses when the target deallocates", ^{
