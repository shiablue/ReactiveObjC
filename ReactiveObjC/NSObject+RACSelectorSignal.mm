...
static BOOL RACForwardInvocation(id self, NSInvocation *invocation) {
	...
	BOOL respondsToAlias = [cls instancesRespondToSelector:aliasSelector];
	if (respondsToAlias) {
...
static void RACSwizzleForwardInvocation(Class cls) {
	SEL forwardInvocationSEL = @selector(forwardInvocation:);
	Method forwardInvocationMethod = class_getInstanceMethod(cls, forwardInvocationSEL);
	...
	class_replaceMethod(cls, forwardInvocationSEL, imp_implementationWithBlock(newForwardInvocation), "v@:@");
}

static void RACSwizzleRespondsToSelector(Class cls) {
	SEL respondsToSelectorSEL = @selector(respondsToSelector:);

	// Preserve existing implementation of -respondsToSelector:.
	Method respondsToSelectorMethod = class_getInstanceMethod(cls, respondsToSelectorSEL);
	...
	id newRespondsToSelector = ^ BOOL (id self, SEL selector) {
		Method method = rac_getImmediateInstanceMethod(cls, selector);
	...
	class_replaceMethod(cls, respondsToSelectorSEL, imp_implementationWithBlock(newRespondsToSelector), method_getTypeEncoding(respondsToSelectorMethod));
}

static void RACSwizzleGetClass(Class cls, Class statedClass) {
...
static void RACSwizzleMethodSignatureForSelector(Class cls) {
	...
	Method methodSignatureForSelectorMethod = class_getInstanceMethod(cls, selector);
	class_replaceMethod(cls, selector, newIMP, method_getTypeEncoding(methodSignatureForSelectorMethod));
}
...
static RACSignal *NSObjectRACSignalForSelector(NSObject *self, SEL selector, Protocol *protocol) {
	...
	@synchronized (self) {
		...
		Class cls = RACSwizzleClass(self);
		NSCAssert(cls != nil, @"Could not swizzle class of %@", self);
		...
		if (targetMethod == NULL) {
			...
			if (!class_addMethod(cls, selector, _objc_msgForward, typeEncoding)) {
				NSDictionary *userInfo = @{
					NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"A race condition occurred implementing %@ on class %@", nil), NSStringFromSelector(selector), cls],
